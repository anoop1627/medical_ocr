...
class PatientDetailsParser(MedicalDocParser):
    ...
    def parse(self):
        return {
            ...
            'medical_problems': self.get_patient_medical_problems(),
            'hepatitus_b_vaccination': self.get_patient_hepatitus_b_vaccination()
        }
    ...
    def get_patient_hepatitus_b_vaccination(self):
        pattern = 'Have you had the Hepatitis B vaccination\?.*(Yes|No)'
        matches = re.findall(pattern, self.text, flags=re.DOTALL)
        if matches:
            return matches[0].strip()

    def get_patient_medical_problems(self):
